...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        new_grads = []
        for param in params:
            new_grads.append((tf.Variable(param), param))
        ...
        def joint_train(avg_grads):
            # for i in range(len(avg_grads)):
                # grad = avg_grads[i][0]
                # new_grads[i][0].assign(grad)
            ...
            # sess.run(_train)
            # # this appears to be a working implementation. I'd like to figure out a way to do it without
            # # having to redo the `global_variables_initializer()` all the time.
            new_grads = []
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            # if we terminated early (finished the level, or died) just pad the episode
